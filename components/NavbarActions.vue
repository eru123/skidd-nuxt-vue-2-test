<template>
  <v-list class="d-flex flex-row" nav :color="$vuetify.theme.dark ? 'dark' : 'primary'">
    <div v-for="(item, key) in navitems" :key="key">
      <v-list-item v-if="item.type == 'link'" :to="item.to" router exact>
        <v-icon>{{ item.icon }}</v-icon>
      </v-list-item>
      <v-menu v-else-if="menu && item.type == 'menu'" open-on-hover bottom offset-y>
        <template v-slot:activator="{ on, attrs }">
          <v-list-item color="primary" dark v-bind="attrs" v-on="on" class="ma-0">
            <v-icon> {{ item.icon }} </v-icon>
          </v-list-item>
        </template>
